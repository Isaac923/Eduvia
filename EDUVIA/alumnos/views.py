...
def lista_alumnos(request):
    ...
    estado_filter = request.GET.get('estado', 'todos')  # Cambiado de 'activo' a 'todos' por defecto
...
def editar_alumno(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, "Error al actualizar el alumno. Por favor revise los datos ingresados.")
    else:
...
def eliminar_alumno(request, pk):
    ...
    if request.method == 'POST':
        nombre_alumno = alumno.nombre_completo  # Guardar el nombre antes de eliminar
        alumno.delete()  # Eliminar el registro en lugar de marcarlo como inactivo
        messages.success(request, f"Alumno {nombre_alumno} eliminado permanentemente.")
        return redirect('alumnos:lista_alumnos')
...
def cambiar_estado_alumno(request, pk):
    if request.method == 'POST':
        ...
        messages.success(request, f'El estado del alumno {alumno.nombre_completo} ha sido actualizado.')

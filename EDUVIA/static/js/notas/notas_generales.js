...
function exportarHistorialPDF() {
    // Intentar obtener el ID del alumno de diferentes fuentes
    let alumnoId = null;
    
    // Primero, intentar obtenerlo de la variable global
    if (currentAlumnoId) {
        alumnoId = currentAlumnoId;
    } else {
        // Si no está en la variable global, intentar obtenerlo del elemento en el DOM
        const nombreAlumnoElement = document.getElementById('historial-alumno-nombre');
        if (nombreAlumnoElement && nombreAlumnoElement.hasAttribute('data-alumno-id')) {
            alumnoId = nombreAlumnoElement.getAttribute('data-alumno-id');
        }
    }
    
    console.log('Exportando PDF para alumno ID:', alumnoId);
    
    if (!alumnoId) {
        mostrarAlertaDjango('No se pudo identificar al alumno para exportar el historial', 'error');
        return;
    }
    ...
    mostrarCargando('Generando PDF del historial...');
    
    // Crear la URL para la solicitud de PDF
    const url = `/notas/exportar-historial-pdf/${alumnoId}/?ano_academico_id=${anoAcademicoId}`;
    
    // Abrir la URL en una nueva pestaña (el navegador manejará la descarga del PDF)
    ...
    if (nuevaVentana) {
        setTimeout(() => {
            ocultarCargando();
            mostrarAlertaDjango('El PDF se está generando y descargando', 'success');
        }, 1500);
    } else {
        ocultarCargando();
        mostrarAlertaDjango('Por favor, permite las ventanas emergentes para descargar el PDF', 'warning');
    }
}
...
function abrirModalEditarNota(alumnoId, materia, semestre, numeroNota, valorActual, anoAcademicoId) {
    ...
    if (valorActual && valorActual !== '--' && valorActual !== '') {
        ...
        if (notaExistente && notaExistente.observaciones) {
            document.getElementById('modal-observaciones').value = notaExistente.observaciones;
            console.log("Observaciones cargadas del cache:", notaExistente.observaciones);
        } else {
            // Si no hay en cache, intentar cargar desde el servidor
            fetch(`/notas/obtener-nota-detalle/${alumnoId}/?materia=${materia}&semestre=${semestre}&numero_nota=${numeroNota}&ano_academico_id=${anoAcademicoId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success && data.nota && data.nota.observaciones) {
                        document.getElementById('modal-observaciones').value = data.nota.observaciones;
                        console.log("Observaciones cargadas del servidor:", data.nota.observaciones);
                        
                        // Actualizar cache
                        if (!notasConPorcentaje[cacheKey]) {
                            notasConPorcentaje[cacheKey] = {};
                        }
                        notasConPorcentaje[cacheKey].observaciones = data.nota.observaciones;
                    }
                })
                .catch(error => {
        ...
        document.getElementById('modal-porcentaje').value = '';
        document.getElementById('modal-observaciones').value = ''; // Limpiar observaciones
    }
...
function guardarNota() {
    // Crear FormData manualmente para asegurar que se incluyan todos los campos
    ...
    const fechaEvaluacion = document.getElementById('modal-fecha').value;
    const observaciones = document.getElementById('modal-observaciones').value;
    const anoAcademicoId = document.getElementById('modal-ano-academico-id').value;
    ...
    // IMPORTANTE: Asegurarse de incluir las observaciones
    formData.append('observaciones', observaciones);
    console.log("Observaciones a guardar:", observaciones);
    
    formData.append('ano_academico_id', anoAcademicoId);
    
    const calificacionOriginal = calificacion;
    const porcentajeOriginal = porcentaje;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            notasConPorcentaje[cacheKey] = {
                ...
                observaciones: observaciones // NUEVO: Guardar observaciones en cache
            };
                    ...
                    if (notaData) {
                        ...
                        if (input) {
                            ...
                            notasConPorcentaje[cacheKey] = {
                                ...
                                observaciones: notaData.observaciones || '' // NUEVO: Guardar observaciones
                            };
...
function verHistorialNotas(alumnoId) {
    ...
    // Guardar el ID del alumno en una variable global para usarlo en la exportación
    currentAlumnoId = alumnoId;
    ...
    fetch(`/notas/historial-notas/${alumnoId}/?ano_academico_id=${anoAcademicoId}`)
        ...
        .then(data => {
            if (data.success) {
                // Almacenar el ID del alumno en el elemento del DOM
                const nombreAlumnoElement = document.getElementById('historial-alumno-nombre');
                if (nombreAlumnoElement) {
                    nombreAlumnoElement.setAttribute('data-alumno-id', alumnoId);
                }
